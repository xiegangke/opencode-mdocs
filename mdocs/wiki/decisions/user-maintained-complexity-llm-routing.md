...
- `mdocs_dispatch` 只组装 initiative、plan、handoff、blockers、Wiki、检索记忆和 audit 上下文，不执行 Task，也看不到当前 TUI 原始输入。
...
对于 model target，config hook 自动生成 `mdocs-route-<binding>`、`mode: subagent` 的静态 Agent，并应用 binding 的 `model`、可选 `variant`、Mdocs 统一执行提示和基础权限。用户无需重复创建 Agent。
...
`mdocs_dispatch` 接口保持不变，当前 TUI 输入由 orchestrator 独立持有。
...
- 文件存在但无效：整份配置失败，不注册任何路由 Agent，也不静默切换到非路由执行。
...
- binding 名称匹配 `^[a-z][a-z0-9-]{0,62}$`；恰好声明 `model` 或 `agent`；`variant` 只能与 `model` 共存；`fallback` 是非法字段。
- agent target 不得使用保留前缀，且必须存在、可作为 subagent 调用并静态绑定模型。

## 可观测性

记录分类等级和理由、最终等级、binding 顺序、实际 binding、从零开始的尝试索引、Agent、模型、配置 hash、基础设施失败分类和 Task 状态。默认不记录完整 TUI 输入，也不根据执行结果修改规则或重排 binding。
